...
- The Python strategy engine is spawned as a child process by Express on startup when `NODE_ENV=production`. Paths are anchored to the running module's location (`artifacts/api-server/dist/index.mjs` → workspace root is `../../..`), not to `process.cwd()`, because the artifact deployment launches Express from `artifacts/api-server/`. The interpreter is resolved in this order: `PYTHON_BIN` env var → workspace-relative `.pythonlibs/bin/python3` (the Nix-wrapped venv interpreter that ships in the deploy image) → `python3` on PATH. The resolved interpreter and cwd are logged at startup so packaging regressions surface in the very first deploy log line. The "started" log only fires after the child's `spawn` event so a failed launch can't print a false success.
- `.pythonlibs/` (the Nix-wrapped python3 + installed strategy-engine deps such as fastapi, grpcio, scikit-learn) is intentionally **not** in `.replitignore`. Excluding it broke the production engine spawn with `ENOENT`, so it is shipped to production despite the ~1.1 GB cost.
